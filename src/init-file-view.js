...
export default InitFileView = class InitFileView extends View {
    ...
    updateContent() {
      ...
      const arr = [];
      for (let k of Object.keys(obj)) { arr.push({name: k, action: obj[k]}); }
    ...
    save() {
      ...
      for (let child of this.package_list.children()) {
        if (child.children[0].classList.contains('icon-diff-added')) { this.file.packages.push({name: child.children[0].innerText, action: 'added'}); }
        if (child.children[0].classList.contains('icon-diff-removed')) { this.file.packages.push({name: child.children[0].innerText, action: 'removed'}); }
      }
