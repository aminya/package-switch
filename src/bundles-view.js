        ...
        return this.li({class: 'two-lines'}, () => {
          ...
          return this.div({class: 'secondary-line'}, () => {
            this.div({class: 'added'}, () => {
              if (actions.added.length !== 0) { return this.span({class: 'icon icon-diff-added'}, actions.added.toString()); }
            });
            return this.div({class: 'removed'}, () => {
              if (actions.removed.length !== 0) { return this.span({class: 'icon icon-diff-removed'}, actions.removed.toString()); }
            });
          });
        });
      });
