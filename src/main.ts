...
export default {
  ...
  activate() {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {'package-switch:stop-packages': () => this.toggle(true)}));
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', { 'package-switch:open-global'() {
    ...
    this.subscriptions.add(atom.commands.add('.tree-view .file .name[data-name$="\\.package-switch\\.cson"]', 'package-switch:open-local', ({target}) => atom.workspace.open(target.dataset.path, {noopener: true}))
    );
    this.subscriptions.add(atom.config.onDidChange('package-switch.SaveRestore', ({newValue}) => { if (newValue) { return this.saveStates(); } }));

    return this.subscriptions.add(atom.workspace.addOpener(function(uritoopen, {noopener}) {
      if (uritoopen.endsWith('.package-switch.cson') && (noopener == null)) {
        if (fs == null) { fs = require('fs'); }
        if (path == null) { path = require('path'); }
        if (this.InitFile == null) { this.InitFile = require('./init-file'); }
        if (this.InitFileView == null) { this.InitFileView = require('./init-file-view'); }
        return this.initfileview = new this.InitFileView({uri: uritoopen, file: new this.InitFile(path.dirname(uritoopen), uritoopen)});
      }
    })
    );
  },
    ...
    if (atom.config.get('package-switch.SaveRestore')) {
      if (atom.config.get('package-switch.InvertSaveData')) {
        ...
        for (let p of atom.packages.getAvailablePackageNames()) {
          if (p === 'package-switch') { continue; }
          if (saveData.includes(p)) { continue; }
          if (lp && p.startsWith('language-')) { continue; }
  ...
  loadProjectConfigs() {
    ...
    if ((p = atom.project.getPaths()).length === 1) {
      ...
      return fs.exists((f = path.join(p[0], '.package-switch.cson')), function(exists) {
        ...
        if (this.InitFile == null) { this.InitFile = require('./init-file'); }
        return setTimeout((() => new this.InitFile(path.basename(p[0]), f).execute(false)), atom.config.get('package-switch.DeferInitialization'));
      });
    } else {
      atom.packages.activatePackage('tree-view');
      atom.packages.activatePackage('tabs');
  ...
  saveStates() {
    return atom.config.set('package-switch.SaveData', atom.config.get('core.disabledPackages').filter((item, index, array) => array.indexOf(item) === index));
  },

  toggle(opposite = false) {
    this.createBundlesInstance();
    this.createBundlesView();
    return this.bundlesview.show(this.bundles.getBundles(), bundle => {
  ...
  createCallback(oldname, items) {
    this.createNameView();
    return this.nameview.show(this.bundles, oldname, items, {confirmCallback: (oldname, name, packages) => {
      return this.nameCallback(oldname, name, packages);
    }
    , backCallback: (oldname, _items) => this.create({
      name: oldname,
      packages: _items
    })});
  },
